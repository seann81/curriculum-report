...
var Mock = (function () {
  ...
  var me = { email: 'admin@iscu.ac.kr', name: '교무팀관리자', role: 'admin', deptCode: '*', deptCodes: ['*'], deptName: '전학과', submitYn: 'Y' };
  // 연습모드(USE_MOCK): 전체 51개 학과 보고서 목록 노출. 실제 DB 반영분과 동일 구성(2026·작성중).
  // [학과코드, 학과명] — DB DEPT 시트와 동일(한글 코드).
  var MOCK_DEPTS = [
    ["교양","WISDOM교양대학"], ["사회복지","사회복지전공"], ["노인복지","노인복지전공"],
    ["복지경영","복지경영전공"], ["아동복지","아동복지전공"], ["청소년복지","청소년복지전공"],
    ["상담심리","상담심리학과"], ["가족코칭","가족코칭상담학과"], ["군경상담","군경상담학과"],
    ["특수심리","특수심리치료학과"], ["심리","심리학과"], ["부동산","부동산학과"],
    ["법무행정","법무행정학과"], ["보건행정","보건행정학과"], ["한국어교육","한국어교육학과"],
    ["안전관리","안전관리학과"], ["경영","경영학과"], ["무역물류","글로벌무역물류학과"],
    ["금융보험","금융보험학과"], ["세무회계","세무회계학과"], ["AI마케팅","AI서비스마케팅학과"],
    ["컴퓨터","컴퓨터공학과"], ["빅데이터보안","빅데이터·정보보호학과"], ["전기전자","전기전자공학과"],
    ["기계제어","기계제어공학과"], ["인공지능","인공지능학과"], ["멀티미디어","멀티미디어디자인학과"],
    ["건축공간","건축공간디자인학과"], ["문예창작","문예창작학과"], ["뷰티","뷰티디자인학과"],
    ["문화예술","문화예술경영학과"], ["음악치료","음악치료학과"], ["실용음악","실용음악과"],
    ["피아노","피아노과"], ["성악","성악과"], ["직업진로","직업진로상담학과"],
    ["AI커머스","AI커머스학과"], ["회화","회화과"], ["모델연기","모델연기학과"],
    ["실용영어","실용영어학과"], ["드론로봇","드론로봇공학과"], ["국방AI","국방AI학과"],
    ["통일안보","통일안보북한학과"], ["AI크리에이터","AI크리에이터학과"], ["AI부동산","AI부동산빅데이터학과"],
    ["AI스마트팜","AI스마트팜학과"], ["통합건강","통합건강관리학과"], ["요가명상","요가명상학과"],
    ["응용통계","응용수학·통계학과"], ["스포츠지도","스포츠지도학과"], ["반려동물","반려동물산업학과"]
  ];
  var reports = MOCK_DEPTS.map(function (d) {
    return { reportId: '2026_' + d[0], year: 2026, deptCode: d[0], deptName: d[1],
